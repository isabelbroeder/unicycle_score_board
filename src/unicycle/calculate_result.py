...
def calculate_result(category: str, age_group:str, judges: dict):
    ...
    df_points = DataLoader(Path(unicycle_score_board_path, "data/points.db"), "points").get_data( f" SELECT * FROM points WHERE category == {category}  AND age_group == {age_goup}")
    ...
    df_sum_points_per_routine_per_judge = pd.DataFrame(
    ...
    for judge in itertools.chain(t_judges, p_judges):
        ...
        df_sum_points_per_routine_per_judge[judge] = df_points[relevant_cols].sum(axis=1)

    print(df_sum_points_per_routine_per_judge)
    print(df_points.columns)
    #percentage of points per routine and judge
    percentage_per_routine_per_judge = df_sum_points_per_routine_per_judge.div(df_sum_points_per_routine_per_judge.sum(axis = 0))
    ...
    for judge_domain in ['T','P']:
        ...
        result[judge_domain] = percentage_per_routine_per_judge[relevant_cols].mean(axis=1)
    print(result)

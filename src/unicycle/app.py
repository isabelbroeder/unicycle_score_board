    ...
    def calculate_result(self, df_points, category, age_group):
        ...
        for judge_domain in ["T", "P"]:
            for judge in category_judges[judge_domain]:
                ...
                if judge_columns:
                    per_judge[judge] = group_df[judge_columns].sum(axis=1, min_count=1)
        ...
        for judge in category_judges["D"]:
            ...
            divisor = 1.0
            if category in ["Small Group", "Large Group", "small_group", "large_group"]:
        ...
        for judge in percentage_per_routine_per_judge.columns:
            ...
            if judge_total == 0:
                percentage_per_routine_per_judge[judge] = np.nan
                percentage_per_routine_per_judge.loc[valid_mask, judge] = 1.0 / valid_mask.sum()
            else:
        ...
        for judge_domain in ["T", "P", "D"]:
            ...
            domain_scores = percentage_per_routine_per_judge[relevant_judges].mean(axis=1, skipna=True)
    ...
    def recalculate_all_results(self, df):
        ...
        if result_frames:
            ...
            df = df.drop(columns=result_cols, errors="ignore").merge(
                all_results, on="id_routine", how="left"
            )
        ...
        table = dash_table.DataTable(
            ...
            style_data_conditional=[
                ...
                *[
                    ...
                    for col in D_COLS if col.startswith(("D3_", "D4_"))
                ],
            ],
        )
